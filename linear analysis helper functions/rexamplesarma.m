...
% 3. Fit of a SARMA model.
% 4. Prediction error statistic for the SARMA model.
% 5. Multi-step predictions from a given target point with the SARMA model.
clear all
...
s = input('Give the period of seasonality >'); 
ps = input('Give the order P of the seasonal AR part >'); 
qs = input('Give the order p of the seasonal MA part >');
[nrmseV,phiallV,thetaallV,SDz,aicS,fpeS]=fitSARMA(xV,p,q,ps,qs,s,Tmax);
fprintf('===== SARMA model ===== \n');
...
% 4. Prediction error statistic for the SARMA model.
nlast = proptest*n;
tittxt = sprintf('SARMA(%d,%d)x(%d,%d)_%d, %%test=%1.2f, prediction error',p,q,ps,qs,s,proptest);
figno = figno + 1;
figure(figno);
clf
[nrmseV,preM] = predictSARMAnrmse(xV,p,q,ps,qs,s,Tmax,nlast,'example');
figno = figno + 1;
figure(figno);
clf
...
figure(figno);
clf
[preV,phiV,thetaV] = predictSARMAmultistep(xV,n1,p,q,ps,qs,s,Tmax,'example');
